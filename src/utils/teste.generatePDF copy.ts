...
import { pagesPDF } from '../pages/resume/download';
...
export const setupCurriculumDownload1 = () => {
...
async function generatePDF3({ pagesContent, filename }: GeneratePDFProps) {
  try {
    if (!pagesContent || pagesContent.length === 0) {
      throw new Error('Nenhum conteúdo de página fornecido.');
    }
    ...
    // Criar uma nova instância do html2pdf para cada página de conteúdo
    const pdf = html2pdf().set(options);
...
export const setupCurriculumDownload3 = () => {
  const curriculumDownload = document.getElementById('btn-download');
